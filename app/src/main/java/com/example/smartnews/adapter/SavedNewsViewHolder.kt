...
fun SavedNews.analyzeMood(context: android.content.Context): String {
    ...
    return when {
        text.containsPositiveKeywords(lang) -> context.getString(R.string.mood_happy)
        text.containsNegativeKeywords(lang) -> context.getString(R.string.mood_sad)
        else -> context.getString(R.string.mood_neutral)
    }
}

private fun String.containsPositiveKeywords(lang: String): Boolean {
    val keywords = if (lang == "ru") {
        setOf(
            "счастливый", "счастье", "радость", "веселый", "веселье", "восторг", "экстаз", "блаженство",
            "эйфория", "ликование", "удовлетворение", "удовольствие", "возбужденный", "возбуждение",
            "энтузиазм", "сияющий", "яркий", "жизнерадостный", "светлый",
            "успех", "успешный", "победа", "победитель", "триумф", "достижение", "прорыв", "превосходство",
            "выдающийся", "замечательный", "награда", "почет", "признание", "аплодисменты", "похвала",
            "рост", "прогресс", "улучшение", "развитие", "инновация", "расширение",
            "процветание", "цветущий", "решение", "прибыль", "выгода", "богатство", "изобилие",
            "любовь", "ласка", "забота", "сострадание", "доброта", "эмпатия", "дружба",
            "единство", "гармония", "мир", "спокойствие", "сотрудничество", "поддержка", "вдохновение",
            "мотивация", "доверие", "лояльность",
            "надежда", "оптимизм", "перспектива", "возможность", "уверенность",
            "мужество", "стойкость", "определенность",
            "празднование", "гордость", "благодарность", "красивый", "замечательный", "удивительный",
            "свобода", "энергия", "благословение"
        )
    } else {
        setOf(
...
private fun String.containsNegativeKeywords(lang: String): Boolean {
    val keywords = if (lang == "ru") {
        setOf(
            "грустный", "грусть", "горе", "печаль", "несчастье", "отчаяние", "депрессия",
            "тоска", "меланхолия", "боль", "страдание", "агония", "дистресс", "мучение",
            "разбитое сердце", "скорбь", "сожаление", "раскаяние", "вина", "стыд",
            "страх", "тревога", "беспокойство", "паника", "террор", "ужас",
            "кризис", "катастрофа", "трагедия", "разрушение", "руины", "хаос",
            "авария", "несчастье", "эпидемия", "пандемия",
            "конфликт", "война", "битва", "насилие", "атака", "терроризм",
            "восстание", "беспорядки", "напряжение", "раздор", "спор",
            "потеря", "смерть", "провал", "неудача", "поражение", "фиаско", "ошибка",
            "слабость", "уязвимость",
            "спад", "падение", "рецессия", "депрессия",
            "банкротство", "долг", "дефицит", "нищета", "безработица", "инфляция",
            "мошенничество", "скандал", "коррупция", "обман", "предательство", "бесчестие",
            "злоупотребление", "несправедливость", "дискриминация", "преследование",
            "проблема", "трудность", "препятствие", "неприятность", "дилемма",
            "нестабильность", "неопределенность", "риск", "опасность", "угроза",
            "нарушение", "страйк", "блокада", "протест",
            "вред", "ущерб", "травма", "разрушить", "саботаж",
            "негативный", "плохой", "ужасный", "страшный", "жестокий", "шокирующий", "тревожный"
        )
    } else {
        setOf(
...
class SavedNewsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind( email: String, savedNews: SavedNews, listener: SavedNewsAdapter.OnNewsDeletedListener?, showDeleteButton: Boolean) {
        ...
        ivDelete.visibility = if (showDeleteButton) View.VISIBLE else View.GONE
